    ...
    public static class UsersDBDAL
    {
        ...
        public static bool RemoveUser(this UsersDBEntities DB, int userId)
        {
            ...
            if (userToDelete != null)
            {
                ...
                foreach(Photo photo in DB.Photos.Where(p => p.UserId == userId))
                {
                    DB.PhotoRatings.RemoveRange(DB.PhotoRatings.Where(pr => pr.PhotoId == photo.Id));
                    photo.Remove();
                }
        ...
        public static bool Remove_PhotoRating(this UsersDBEntities DB, PhotoRating photoRating)
        {
            PhotoRating existingPhotoRating = DB.PhotoRatings.Where(pr => pr.PhotoId == photoRating.PhotoId && pr.UserId == photoRating.UserId).FirstOrDefault();
            if (existingPhotoRating != null)
            {
                DB.PhotoRatings.Remove(existingPhotoRating);
                DB.SaveChanges();
            }
